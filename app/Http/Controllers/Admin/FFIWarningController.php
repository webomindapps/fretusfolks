...
class FFIWarningController extends Controller
{
    ...
    {
        $searchColumns = ['id', 'date', 'emp_id', 'emp_name', 'phone1', 'designation'];
        ...
        if ($search != '') {
            $query->where(function ($q) use ($search, $searchColumns) {
                foreach ($searchColumns as $key => $value) {
                    if (in_array($value, ['emp_name', 'phone1', 'designation'])) {
                        $q->orWhereHas('warning_letter', function ($q2) use ($value, $search) {
