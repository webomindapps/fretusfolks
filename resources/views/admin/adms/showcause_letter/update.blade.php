<x-applayout>
    <x-admin.breadcrumb title="Edit ADMS ShowCause Letter" />
    ...
    <div class="col-lg-12 pb-4">
        <div class="form-card px-3px-md-3 px-2">
            <form action="{{ route('admin.showcause_letter.edit', $showcause->id) }}" method="POST" enctype="multipart/form-data"
                id="employeeForm">
                @csrf
                <div class="card mt-3">
                    <div class="card-header header-elements-inline d-flex justify-content-between align-items-center">
                        <h5 class="card-title mb-0">Edit Show Cause Letter Details</h5>
                    </div>
                    <div class="card-body">
                            ...
                            <div class="row">
                                <div class="col-lg-6 mt-2">
                                    ...
                                    <input type="text" id="emp_id" name="emp_id"
                                        value="{{ $showcause->emp_id }}" onchange="return fetchEmployeeDetails()" required>
                                    <input type="hidden" id="fetchEmployeeRoute"
                                        data-url="{{ route('admin.showcause_letter.details', ['emp_id' => ':emp_id']) }}">
                                </div>
                                <x-forms.input label="Enter Employee Name: " type="text" name="emp_name"
                                    id="emp_name" :required="true" size="col-lg-6 mt-2" :value="$showcause->showcauseletter->emp_name" readonly />
                                <x-forms.input label="Designation: " type="text" name="designation" id="designation"
                                    :required="true" size="col-lg-6 mt-2" :value="$showcause->showcauseletter->designation" readonly />
                                <x-forms.input label="Date: " type="date" name="date" id="date"
                                    :required="false" size="col-lg-6 mt-2" :value="$showcause->date" />
                               
                                <x-forms.textarea label="Content: " type="text" name="content" id="content"
                                    :required="true" size="col-lg-12 mt-2" :value="$showcause->content" />
                            </div>
                            <div class="row">
